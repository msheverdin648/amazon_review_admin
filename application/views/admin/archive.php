<div class="page-table">
        ...
        <div class="page-table__tabs"><a class="page-table__tabs-link " href="/admin/table/<?=$reviewer['opinion']?>">Активные (<?=$out?>)</a><a class="page-table__tabs-link page-table__tabs-link--active" >Архив (<?=$in?>)</a></div>
                        ...
                        <? foreach($form_query as $form): ?>
                            <?if($form['archive']==1):?>
                                <tr class="table-item">
                                  ...
                                  <td class="text"><?=$form['star']?><br><br><? if($form['review'] != 'null'): ?><?=$form['review']?><?endif;?><? if(!empty($form['img'])): ?><a href="/admin/fetch_image/<?=$form['id']?>" target="_blank"> https://eazymatcha.com/admin/fetch_image/<?=$form['review_id']?> </a><? elseif(!empty($form['link'])):?><a href="<?=$form['link']?>"><?=$form['link']?></a> <? endif; ?></td>
